...
mkdir -p results/end_motifs/WGS_5mer/all_clean
mkdir -p logs/wgs_endmotif_5mer
...
while IFS=$'\t' read -r sample group; do
  ...
  OUT5="results/end_motifs/WGS_5mer/all_clean/${sample}_5prime_counts.txt"
  OUT3="results/end_motifs/WGS_5mer/all_clean/${sample}_3prime_counts.txt"
  ...
  echo "[$(date)] DONE sample=$sample group=$group"
done < "$GROUP_MANIFEST"
