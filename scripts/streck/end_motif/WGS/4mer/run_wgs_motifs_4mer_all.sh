...
mkdir -p results/end_motifs/WGS/all_raw
mkdir -p results/end_motifs/WGS/all_clean
...
while read sample group dtype status; do
  ...
  if [ "$dtype" = "WGS" ]; then
    ...
    OUT_PREFIX="results/end_motifs/WGS/all_raw/${sample}"
    ...
    # clean + count (remove N)
    grep -v N ${OUT_PREFIX}_5prime.txt | sort | uniq -c | sort -nr \
    > results/end_motifs/WGS/all_clean/${sample}_5prime_counts.txt

    grep -v N ${OUT_PREFIX}_3prime.txt | sort | uniq -c | sort -nr \
    > results/end_motifs/WGS/all_clean/${sample}_3prime_counts.txt
