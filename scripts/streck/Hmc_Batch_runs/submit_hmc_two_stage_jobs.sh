...
while read -r sample group dtype status; do
  ...
  jid1=$(bsub \
    ...
    -M 64000000 \
    -o "logs/hmc_stage1/${sample}.out" \
    -e "logs/hmc_stage1/${sample}.err" \
    scripts/hmc_stage1_bwa_worker.sh "$sample" "$group" \
    | awk '{gsub(/[<>]/,"",$2); print $2}')
  ...
  bsub \
    ...
    -M 16000000 \
    -o "logs/hmc_stage2/${sample}.out" \
    -e "logs/hmc_stage2/${sample}.err" \
    scripts/hmc_stage2_samtools_worker.sh "$sample" "$group"

done < manifests/hmc_manifest_balanced.tsv
